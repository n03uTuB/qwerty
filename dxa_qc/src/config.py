...
# Источник скора по каждому критерию: "cnn" — чистая нейросеть, "fused" —
# гибрид CNN + геометрия (stack.py). Выбор сделан по F1@prior, а НЕ по AUC:
# в режиме prior порог ставится по доле нарушений, поэтому решение целиком
# определяется ранжированием в top-k (F1 = 2*TP/(n_pos+k)). Проверено стендом
# scripts/eval_source_configs.py (перебор 3^5 комбинаций):
#   * гибрид проигрывает сети на бедре: femur_positioning F1@prior 0.361 -> 0.444,
#     femur_roi 0.571 -> 0.714;
#   * гибрид нужен только для оси позвоночника, где CNN антикоррелирован
#     (AUC 0.383, F1@prior 0.100 против 0.300 у геометрии);
#   * macro-F1 организатора: 0.407 (гибрид везде) -> 0.461 (гибрид только для оси).
# Все 8 лучших комбинаций из 243 дают ровно этот паттерн.
CRITERION_SOURCES = {
    "spine_positioning": "cnn",
    "spine_axis": "fused",
    "spine_artifacts": "cnn",
    "femur_positioning": "cnn",
    "femur_roi": "cnn",
}
