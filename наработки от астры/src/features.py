...
Идея: критерии ТЗ измеримы, поэтому их надо мерить, а не угадывать сетью.
Признаки перенесены из честного измерительного контура (``dxa_real/features.py``)
и подобраны под каждый критерий по CV (см. :data:`src.stack.CRITERION_FEATURES`).

Позвоночник:
  * ось — угол средней линии столба (``spine_midline_angle``), допуск 5 град;
    + остаточная кривизна ``spine_midline_residual`` — ключевой признак v3:
    отличает «наклон от прямой оси» от физиологического изгиба;
...
Бедро:
  * ROI — отступы поля сканирования (3 см сверху/снизу, 2 см сбоку);
  * ротация/укладка — малый вертел и ширина шейки.

Линейные/угловые величины считаются в физических единицах по реальному размеру
пикселя (spacing из Exposed Area, см. :mod:`src.dicom_io`).
"""
...
FEATURE_NAMES = [
    ...
    # «лишнее» вне кости (критерий «предметы»): яркие объекты, резкие края, тёмное
    "art_max_out", "art_p999_out",
    "art_bright_out_frac", "art_bright_out_maxcc", "art_bright_out_ncc",
    "art_bright_out_edge", "art_edge_out_mean", "art_edge_out_p99",
    "art_edge_out_p999", "art_dark_in_frac",
    # атлас-регистрация (укладка + предметы): остаток к шаблону и параметры
    "atlas_resid_out", "atlas_resid_out_p99", "atlas_outlier_area",
    "atlas_resid_bone", "atlas_bone_iou", "align_angle", "align_scale",
    # атлас бедра (укладка бедра): отдельный шаблон
    "fatlas_resid_out", "fatlas_resid_out_p99", "fatlas_outlier_area",
    "fatlas_resid_bone", "fatlas_bone_iou", "falign_angle", "falign_scale",
    # бедро
    ...
    "femur_bbox_h_ratio", "femur_head_dx",
    # общие
...
def largest_component(mask: np.ndarray, prefer_bottom: bool = False) -> np.ndarray:
    """Самая крупная связная кость; с prefer_bottom — доходящая до низа кадра."""
    from scipy import ndimage
...
def _midline_from_mask(mask: np.ndarray, spacing):
    """Наклон средней линии столба по костной маске (как меряет врач).

    Возвращает (угол к вертикали в град., остаточный разброс).
    """
    rows, cols = mask.shape
...
def medial_side(mask: np.ndarray) -> str:
    """Медиальная сторона по анатомии: головка смещена медиально от диафиза."""
    ys, xs = np.nonzero(mask)
    if len(ys) < 50:
        return "left"
...
def _shaft_and_trochanter(mask: np.ndarray, medial_side: str, spacing) -> dict:
    """Малый вертел как выступ медиального контура над линией диафиза."""
    ys, xs = np.nonzero(mask)
    if len(ys) < 50:
        return dict(femur_trochanter_bulge=0.0, femur_shaft_deg=0.0,
                    femur_shaft_width_cm=0.0)
    y0, y1 = int(ys.min()), int(ys.max())
    height = y1 - y0
    ...
    shaft_rows = [r for r in range(int(y0 + height * 0.7), y1 + 1)
...
def femur_features(arr: np.ndarray, spacing=C.PIXEL_SPACING_MM) -> dict:
    ...
    # вертикальный размах кости в кадре и смещение головки от центра кости —
    # прямые меры масштаба/центровки укладки бедра
    ys, xs = np.nonzero(mask)
    if len(ys) >= 50:
        y0, y1 = int(ys.min()), int(ys.max())
        bbox_h = float((y1 - y0) / max(rows, 1))
        top = ys < y0 + (y1 - y0) * 0.25
        head_dx = (float(abs(xs[top].mean() - xs.mean()) / max(cols, 1))
                   if top.any() else 0.0)
    else:
        bbox_h = head_dx = 0.0
    ...
    feats = dict(
        ...
        femur_bbox_h_ratio=bbox_h,
        femur_head_dx=head_dx,
    )
...
def compute_features(arr: np.ndarray, spacing=C.PIXEL_SPACING_MM,
                     region=None, copies: float = 1.0, atlas=None,
                     femur_atlas=None) -> dict:
    """Полный вектор геометрических признаков для изображения.

    region: если известна, считается только нужная группа (остальные — 0).
    atlas:  :class:`src.atlas.Atlas` (необязательно) — включает atlas_*/align_*.
    """
    ...
    if is_spine:
        ...
        # «лишнее» вне кости + сравнение с шаблоном — только для позвоночника
        from .atlas import artifact_features, atlas_features
        out.update(artifact_features(arr, spacing=(sx, sy)))
        out.update(atlas_features(arr, atlas, spacing=(sx, sy)))
    if is_femur:
        out.update(femur_features(arr, spacing=(sx, sy)))
        # сравнение с шаблоном бедра (укладка бедра)
        from .atlas import atlas_features
        out.update(atlas_features(arr, femur_atlas, spacing=(sx, sy),
                                  region="femur"))
...
def _load_for_features(row, verbose: bool = False) -> np.ndarray:
    """Загрузить изображение для признаков.

    Признаки НЕ инвариантны к нелинейным преобразованиям: кэш PNG хранит
    изображение после modality/VOI LUT (отсечение окном) — на нём признаки
    смещаются. Поэтому для реальных строк читаем исходный DICOM, а для
    синтетических — кэш (нарушение лежит только там).
    """
...
def build_features(manifest_csv: str = C.MANIFEST_CSV,
    ...
    femur_atlas = None
    try:
        from .atlas import Atlas, atlas_path
        atlas = Atlas.load(atlas_path("spine"))
        if atlas is not None:
            print("[features] атлас позвоночника загружен")
        femur_atlas = Atlas.load(atlas_path("femur"))
        if femur_atlas is not None:
            print("[features] атлас бедра загружен")
    except Exception as e:
        print("[features] атлас не загружен:", e)
    rows = []
    for _, r in df.iterrows():
        ...
        copies = (float(r["copies"]) if "copies" in df.columns
                  and pd.notna(r.get("copies")) else 1.0)
        feat = compute_features(arr, spacing, region=r.get("region"),
                                copies=copies, atlas=atlas,
                                femur_atlas=femur_atlas)
